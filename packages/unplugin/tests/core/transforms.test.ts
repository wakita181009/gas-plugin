...
describe("stripExportKeywords", () => {
  ...
  it("strips export from class declarations", () => {
    const input = "export class MyService {\n  run() {}\n}";
    const result = stripExportKeywords(input);
    expect(result).toBe("class MyService {\n  run() {}\n}");
  });

  it("strips export from abstract class declarations", () => {
    const input = "export abstract class Base {\n  abstract run(): void;\n}";
    const result = stripExportKeywords(input);
    expect(result).toBe("abstract class Base {\n  abstract run(): void;\n}");
  });
});
